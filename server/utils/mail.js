...
async function sendPasswordResetEmail(email, token) {
  ...
  if (isHttpApi) {
    try {
      ...
      console.log(`[Brevo API] ✅ Reset email sent via HTTP to ${email}`);
      return { success: true };
    } catch (err) {
      console.error("[Brevo API] ❌ HTTP API error:", err.message);
      console.log(`[Brevo MOCK FALLBACK] Reset link for ${email}: ${link}`);
      return { success: true, fallback: true };
    }
  }
  ...
  try {
    ...
    console.log(`[Brevo SMTP] ✅ Reset email sent to ${email}`);
    return { success: true };
  } catch (err) {
    console.error("[Brevo SMTP] ❌ sendPasswordResetEmail error:", err.message);
    console.log(`[Brevo MOCK FALLBACK] Reset link for ${email}: ${link}`);
    return { success: true, fallback: true };
  }
}
