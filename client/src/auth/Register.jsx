...
export default function Register({ defaultRole = "client" }) {
  ...
  const [emailToken, setEmailToken] = useState("");  // short-lived token from /verify-email
  ...
  // Verify OTP for registration (uses /verify-email, not /verify-otp which creates a session)
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (res.data.success && res.data.emailToken) {
        setOtpVerified(true);
  ...
  const handleRegister = async () => {
    ...
    try {
      const userData = {
        ...formData,
        emailToken,   // server validates this to enforce email verification
        plan: "free",
        location: {
