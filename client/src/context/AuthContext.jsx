...
export function AuthProvider({ children }) {
  ...
  // Only store non-sensitive UI fields in localStorage to limit PII exposure on XSS
  ...
  const login = async (email, password) => {
    try {
      ...
      // Store minimal UI metadata; actual auth is via httpOnly cookie set by server
      localStorage.setItem("user", JSON.stringify(toSafeUser(user)));
  ...
  const loginWithToken = (user, token) => {
    // token is set as httpOnly cookie by server; store only safe UI metadata
    localStorage.setItem("user", JSON.stringify(toSafeUser(user)));
    setUser(user);
  };
  ...
  const updateUser = (userData) => {
    localStorage.setItem("user", JSON.stringify(toSafeUser(userData)));
    setUser(userData);
  };
